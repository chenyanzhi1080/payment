...
public class DateTimeUtil {
	...
	public static int getDayCount(final String date1, String date2) {
		...
		int dayCount1 = DateTimeUtil.getDayCount(date1);
		int dayCount2 = DateTimeUtil.getDayCount(date2);
	...
	public static boolean isToday(String time) {
		...
		if (DateTimeUtil.getDate().equals(time)) {
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public static synchronized Date parseYYYYMMddHHmmss(String dateStr){
		try {
			return sdf.parse(dateStr);
		} catch (ParseException e) {
			throw new ErrorCodeException(ErrorCode.PARAM.getErrorCode(), "日期格式不正确");
		}
	}
	
	
	public static synchronized Date parse(String dateStr, String patter){
		try {
	...
	public static Date toDate(String datetime) {
		...
		long time = DateTimeUtil.getTimestamp(datetime);
		if (time <= 0) {
	 ...
	 * @return
	 */
	public static String getDayHourMinuteSecond(Long millis){
		StringBuffer buffer = new StringBuffer();
		Long day = millis/DAY_MILLIS;
		Long hours = millis%DAY_MILLIS/ HOUR_MILLIS;
		Long minutes = millis%DAY_MILLIS% HOUR_MILLIS/MINUTE_MILLIS;
		Long seconds = millis%DAY_MILLIS%HOUR_MILLIS%MINUTE_MILLIS/1000;
		if(day>0){
			buffer.append(day).append("天");
		}
		
		buffer.append(hours>0?hours:0).append("小时");
		buffer.append(minutes>0?minutes:0).append("分");
		buffer.append(seconds>0?seconds:0).append("秒");
		
		return buffer.toString();
	}
	
	public static long getUnixTimestamp(long millTime){
		return millTime / 1000;
	}
	
	/**
	 * 判断date是否是在date1之后或者是相等
	 * @param date
	 * @param date1
	 * @return
	 */
	public static boolean afterOrEqua(Date date, Date date1){
		return date.getTime() >= date1.getTime();
	}
	
	/**
	 * 判断date是否是在date1之前或者是相等
	 * @param date
	 * @param date1
	 * @return
	 */
	public static boolean beforeOrEqua(Date date, Date date1){
		return date.getTime() <= date1.getTime();
	}
